    ...
    return (
        <div className='sidebar hide-on-large-only'>
                ...
                <div className="site-logo-sm col s6 pull-s2">
                    <div style={{ position: "absolute", top: "0px", left: "10px", width: "15rem", height: "50px" }}>
                        <Link to="/" style={{ textDecoration: "none" }}>
                            <img src={Logo} className="logo" alt="" /> <p style={{ position: "absolute", marginTop: "6px", marginLeft: "45px", fontWeight: "bold", fontSize: "22px" }}>Dulge</p>
                        </Link>
                    </div>
                </div>
            </div>
            ...
            <ul id="slide-out" className="sidenav white">
                ...
                <li>{user?.email ? (<Link to="/Dashboard" className="btn login_btn z-depth-0 panel_btn" style={{ border: "none !important" }}>Panel</Link>) : (<Link to="/signin" className="btn login_btn z-depth-0">Log In</Link>)}</li>
                <li>{user?.email ? (<a className="btn start_btn z-depth-0" onClick={handleSignOut}>Logout</a>) : (<Link to="/signup" className="btn start_btn z-depth-0">Start for Free</Link>)}</li>
            </ul>
        </div>
    );
}
